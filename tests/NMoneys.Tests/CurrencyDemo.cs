...
public class CurrencyDemo
{
	...
	{
		Assert.That(Currency.TryGet(CurrencyIsoCode.ZAR, out Currency? currency), Is.True);
	...
	{
		CurrencyIsoCode notDefined = (CurrencyIsoCode)0;
		Assert.That(Currency.TryGet(notDefined, out Currency? currency), Is.False);
	...
	public void Currency_instances_follow_a_flightweight_like_pattern_being_exactly_equal()
	{
		...
		Currency.TryGet("UYU", out Currency? pesoUruguayo);
	...
	public void whenever_a_deprecated_currency_is_obtained_an_event_is_raised()
	{
		...
		// DO unsubscribe from global events whenever listening is not needed anymore
		finally
		{
			Currency.ObsoleteCurrency -= callback;
		}
	...
	public void some_currencies_have_an_character_reference_for_angly_bracket_languages()
	{
		...
		Assert.That(reference?.NamedReference, Is.EqualTo("&euro;"));
		Assert.That(reference?.DecimalReference, Is.EqualTo("&#8364;"));
		Assert.That(reference?.Name, Is.EqualTo("euro"));
	}
