...
public class MakeOptimalChangeTester
{
	[Test]
		...
		public void MakeOptimalChange_NoDenominations_EmptySolution()
		{
			...
			var emptySolution = subject.MakeOptimalChange(Array.Empty<Denomination>());

			Assert.That(emptySolution, Iz.NotOptimalChange());
		}
		...
		[Test]
		public void MakeOptimalChange_IdentityChange_OneDenomination()
		{
			var subject = new Money(5m);
			var change = subject.MakeOptimalChange(5m);

			Assert.That(change, Iz.OptimalChange(1.x(5)));
		}

		[Test]
		public void MakeOptimalChange_ChangePossible_MultipleDenominations()
		{
			var subject = new Money(5m);
			var wholeSolution = subject.MakeOptimalChange(1m, 3m, 2m);

			Assert.That(wholeSolution, Iz.OptimalChange(1.x(3), 1.x(2)));
		...
		public void MakeOptimalChange_NonOptimalForGreedy_OptimalSolution(decimal amount, decimal[] denominationValues, QDenomination[] solution)
		{
			...
			Assert.That(optimalSolution, Iz.OptimalChange(solution));
		}
		...
		public void MakeOptimalChange_NotCompleteSolution_EmptySolution()
		{
			...
			Assert.That(incomplete,Iz.NotOptimalChange());
		}
