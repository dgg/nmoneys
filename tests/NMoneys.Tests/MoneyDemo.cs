...
public class MoneyDemo
{
	...
	public void some_currencies_are_divided_into_more_than_100_units()
	{
		...
		Assert.That(.007m.ToMoney(dinar).MinorIntegralAmount, Is.EqualTo(7L),
			"for fractional amounts, the minor unit prevails");
	...
	public void and_some_currencies_do_not_have_minor_units()
	{
		...
		Assert.That(3.7m.ToMoney(yen).MinorIntegralAmount, Is.EqualTo(3L),
			"for fractional amounts, decimals are discarded");
	...
	[Test]
	public void using_different_styles_for_currencies_that_span_multiple_countries()
	{
		...
		string threeThousandAndTheHalfInFrench =
			string.Format("3{0}000,50 €", french.NumberFormat.CurrencyGroupSeparator);
		Assert.That(3000.5m.Eur().ToString(french), Is.EqualTo(threeThousandAndTheHalfInFrench));
	}
	...
	public void several_unary_operations_can_be_performed()
	{
		...
		Assert.That(.5m.ToMoney().RoundToNearestInt(MidpointRounding.ToEven), isMoneyWith(0m),
			"closest even number is 0");
		Assert.That(.5m.ToMoney().RoundToNearestInt(MidpointRounding.AwayFromZero), isMoneyWith(1m),
			"closest number away from zero is 1");
		Assert.That(1.5m.ToMoney().RoundToNearestInt(MidpointRounding.ToEven), isMoneyWith(2m),
			"closest even number is 2");
		Assert.That(1.5m.ToMoney().RoundToNearestInt(MidpointRounding.AwayFromZero), isMoneyWith(2m),
			"closest number away from zero is 2");
	...
	{
		Assert.That(Money.Parse("$1.5", Currency.Dollar), isMoneyWith(1.5m, CurrencyIsoCode.USD),
			"one-and-the-half dollars");
		Assert.That(Money.Parse("10 €", Currency.Euro), isMoneyWith(10m, CurrencyIsoCode.EUR), "ten euros");

		Assert.That(Money.Parse("kr. -100", Currency.Dkk), isMoneyWith(-100m, CurrencyIsoCode.DKK),
			"owe hundrede kroner");
		Assert.That(Money.Parse("(¤1.2)", Currency.None), isMoneyWith(-1.2m, CurrencyIsoCode.XXX),
			"owe one point two, no currency");
	}
	...
	public void number_formatting_is_flexible_for_parsing_purposes()
	{
		...
		Assert.That(Money.Parse("-10", Currency.Euro), isMoneyWith(-10m, CurrencyIsoCode.EUR),
			"negative euro style without no symbol");
		Assert.That(Money.Parse("(1.2)", Currency.Euro), isMoneyWith(-12m, CurrencyIsoCode.EUR),
			"negative dollar style with no symbol");
	}

	[Test]
	public void currencies_have_to_be_compatible_for_parsing()
	{
		Assert.That(() => Money.Parse("($1.5)", Currency.Eur), Throws.InstanceOf<FormatException>(),
			"currency not compatible");
