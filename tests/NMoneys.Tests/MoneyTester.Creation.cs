...
{
	private static readonly Money oweMeQuid = new (-1, Currency.Gbp);
	...
	public void ForMajor_Currency()
	{
		...
		Assert.That(Money.ForMajor(234, Currency.Gbp), Iz.MoneyWith(234, Currency.Gbp));
	}

	[TestCaseSource(nameof(forMinorAllowingDecimals))]
	public void ForMinor_Currency_AllowingDecimals_AmountShiftedAsManyDigitsAsCurrencySpecifies(Currency currency,
		int decimalDigits, long minorAmount, decimal amount)
	{
		Assert.That(currency.SignificantDecimalDigits, Is.EqualTo(decimalDigits));
		Assert.That(Money.ForMinor(minorAmount, currency), Iz.MoneyWith(amount, currency));
	}
	...
	public void ForMinor_Currency_NotAllowingDecimals_AmountStaysAtItis()
	{
		...
		Assert.That(Money.ForMinor(234, notAllowingDecimals), Iz.MoneyWith(234m, notAllowingDecimals));
	}
	...
	{
		Assert.That(Money.Zero(), Iz.MoneyWith(decimal.Zero, Currency.None));
	}

	[Test]
	public void Zero_ExistingIsoCode_PropertiesSet()
	{
		Assert.That(Money.Zero(CurrencyIsoCode.USD), Iz.MoneyWith(decimal.Zero, Currency.Usd));
	}
	...
	public void Zero_Array_ExistingIsoCode_CollectionOfZeroes()
	{
		Assert.That(Money.Zero(CurrencyIsoCode.USD, 3), Has.Length.EqualTo(3).And
			.All.Matches(Iz.MoneyWith(decimal.Zero, Currency.Usd)));
	}

	[Test]
	public void Zero_ExistingIsoSymbol_PropertiesSet()
	{
		Assert.That(Money.Zero("EUR"), Iz.MoneyWith(decimal.Zero, Currency.Euro));
	}
	...
	public void Zero_Array_ExistingIsoSymbol_CollectionOfZeroes()
	{
		Assert.That(Money.Zero("EUR", 2), Has.Length.EqualTo(2).And
			.All.Matches(Iz.MoneyWith(decimal.Zero, Currency.Euro)));
	}
	...
	public void Zero_NonExistingIsoCode_Exception()
	{
		...
		Assert.That(() => Money.Zero(nonExistingCode),
			Throws.InstanceOf<UndefinedCodeException>().With.Message.Contains("7"));
	}
