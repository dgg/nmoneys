...
public partial struct Money
{
		...
		public Money(decimal amount, Currency currency) : this(amount, Ensuring.NotNull(nameof(currency), currency, c => c.IsoCode)) { }
...
#pragma warning restore 612,618
	...
	/// <summary>
		...
		{
			decimal minorAmount = decimal.Truncate(decimal.Multiply(amount, SmallPowerOfTen.Positive(currency.SignificantDecimalDigits)));
			return minorAmount;
		}
		...
		{
			if (!HasSameCurrencyAs(money)) throw DifferentCurrencyException.ForCodes(CurrencyCode.ToString(), money.CurrencyCode.ToString());
		}
