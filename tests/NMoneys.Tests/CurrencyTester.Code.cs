...
public partial class CurrencyTester
{
	#region Parse
		...
		{
			Assert.That(() => Currency.Code.Parse("notAnIsoCode", CurrencyIsoCode.XTS), Throws.InstanceOf<UndefinedCodeException>());
		}
		...
		{
			long overflowingCode = short.MinValue + 1L;
			Assert.That(() => Currency.Code.Parse(overflowingCode.ToString(), CurrencyIsoCode.XTS), Throws.InstanceOf<UndefinedCodeException>());
		}
