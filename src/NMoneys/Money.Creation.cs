...
public partial struct Money
{
	...
	/// Thus 'USD 20' can be initialized, but not the value 'USD 20.32'.</remarks>
	/// <param name="amountMajor">The <see cref="Amount"/> in the major division of the currency.</param>
	/// <param name="currency">The <see cref="CurrencyCode"/> of the monetary quantity (<see cref="CurrencyIsoCode.XXX"/> if not passed).</param>
	/// <returns>A <see cref="Money"/> with the specified <paramref name="amountMajor"/> and <paramref name="currency"/>.</returns>
	/// <seealso cref="Money(decimal, CurrencyIsoCode)"/>
	[Pure]
	public static Money ForMajor(long amountMajor, CurrencyIsoCode currency = CurrencyIsoCode.XXX)
	{
	...
	[Pure]
	public static Money ForMinor(long amountMinor, CurrencyIsoCode currency = CurrencyIsoCode.XXX)
	{
	...
	public static Money ForMinor(long amountMinor, Currency currency)
	{
		...
		return new Money(
			decimal.Divide(amountMinor, SmallPowerOfTen.Positive(currency.SignificantDecimalDigits)),
			currency);
	}
