...
namespace NMoneys.Tests
{
	...
	public partial class MoneyTester
	{
		...
		{
			NMoneys.Allocation.Allocation allocated = 8m.Usd().Allocate(4, allocator);
		...
		{
			NMoneys.Allocation.Allocation allocated = 8.3m.Usd().Allocate(4, RemainderAllocator.FirstToLast);
		...
		{
			NMoneys.Allocation.Allocation allocated = 8.3m.Usd().Allocate(4, RemainderAllocator.LastToFirst);
		...
		{
			NMoneys.Allocation.Allocation allocated = 8.3m.Eur().Allocate(4, RemainderAllocator.Random);
		...
		{
			NMoneys.Allocation.Allocation allocated = 8.3m.Eur().Allocate(1, remainder);
		...
		public void Allocate_RemainderAllocatorDidNotAllocateAllTheRemainder_IncompleteAllocation()
		{
			...
			NMoneys.Allocation.Allocation allocated = 8.3m.Gbp().Allocate(4, rogue);
		...
		public void Allocate_WholeCurrencies_WholeAllocationsArePossible()
		{
			...
			NMoneys.Allocation.Allocation allocated = 34m.Jpy().Allocate(4, RemainderAllocator.FirstToLast);
		...
		{
			var threeYen = 3m.Jpy();
			NMoneys.Allocation.Allocation allocated = threeYen.Allocate(4, RemainderAllocator.FirstToLast);
		...
		{
			var threeYen = 3m.Jpy();
			NMoneys.Allocation.Allocation allocated = threeYen.Allocate(2, RemainderAllocator.LastToFirst);
		...
		public void Allocate_NotEvenTheMinimumForEveryone_QuasiCompleteAllocation()
		{
			...
			NMoneys.Allocation.Allocation allocated = notEvenAYen.Allocate(2, RemainderAllocator.FirstToLast);
		...
		public void Allocate_ResidualRemainders_Exception()
		{
			...
			NMoneys.Allocation.Allocation allocated = residualRemainder.Allocate(2, RemainderAllocator.FirstToLast);
